...
class MappingService:
    ...
    async def load_character_colors_from_db(self, session: AsyncSession):
        """Load character colors from database"""
        try:
            ...
            for row in result.fetchall():
                ...
                if tel: self.character_colors_map[tel] = hexcol
        except Exception as e:
            print(f"[ERROR] Loading character colors failed: {e}")
    async def get_color_mappings(self, session: AsyncSession):
        """Get all color mappings from database"""
        try:
            result = await session.execute(text("""
                SELECT
                    id,
            ...
            print(f"[ERROR] get_color_mappings failed: {e}")
            return []

    async def closest_character(self, session: AsyncSession, r: int, g: int, b: int):
        """
        ...
        try:
            ...
            for row in mappings:
                ...
                dist = ((row_dict["color_r"] - r) ** 2 +
                        (row_dict["color_g"] - g) ** 2 +
                        (row_dict["color_b"] - b) ** 2) ** 0.5
                if dist < min_dist:
                    min_dist = dist
                    closest_char = row_dict["devanagari_char"]
                    closest_shade = row_dict["colour_shade"]
                    closest_sthana = row_dict["sthana"]
    ...
    async def load_initial_values_from_db(self, session: AsyncSession):
        """Loads all mappings from DB into memory."""
        try:
            # Load consonant mappings
            result = await session.execute(text("SELECT latin_character, char_id, devanagari_character FROM consonants_latin"))
            for row in result.fetchall():
                latin_char, code, dev_char = row
            ...
            # Load vowel mappings
            result = await session.execute(text("SELECT latin_character, char_id, devanagari_character FROM vowels_latin"))
            for row in result.fetchall():
                latin_char, code, dev_char = row
            ...
            result = await session.execute(text("SELECT devanagari_char, latin_equivalent_vowel FROM devanagari_matras_map"))
            for row in result.fetchall():
                dev_matra_char, latin_vowel_eq = row
                self.DEVANAGARI_MATRA_TO_LATIN_VOWEL[
                    unicodedata.normalize('NFC', dev_matra_char)
                ] = latin_vowel_eq

            # Load Devanagari modifiers
            result = await session.execute(text("SELECT devanagari_char, latin_equivalent_modifier FROM devanagari_modifiers_map"))
            for row in result.fetchall():
                dev_mod_char, latin_mod_eq = row
                self.DEVANAGARI_MODIFIERS_TO_LATIN[
                    unicodedata.normalize('NFC', dev_mod_char)
                ] = latin_mod_eq

            # Load Kannada mappings
            await self._load_kannada_mappings_from_db(session)

            # Load Telugu mappings
            await self._load_telugu_mappings_from_db(session)

        except Exception as err:
            print(f"[ERROR] DB load failed: {err}")

    async def _load_kannada_mappings_from_db(self, session: AsyncSession):
        """Loads Kannada-specific mappings."""
        try:
            # Consonants
            result = await session.execute(text("SELECT kannada_character, devanagari_character FROM consonants_kannada"))
            for row in result.fetchall():
                kannada_char, dev_char = row
            ...
            # Vowels
            result = await session.execute(text("SELECT kannada_character, devanagari_character FROM vowels_kannada"))
            for row in result.fetchall():
                kannada_char, dev_char = row
            ...
            # Matras
            result = await session.execute(text("SELECT kannada_char, devanagari_char FROM kannada_matras_map"))
            for row in result.fetchall():
                kannada_matra, dev_matra = row
            ...
            # Modifiers
            result = await session.execute(text("SELECT kannada_char, devanagari_char FROM kannada_modifiers_map"))
            for row in result.fetchall():
                kannada_mod, dev_mod = row
    ...
    def _load_tamil_mappings_from_db(self, session: AsyncSession):
        """Loads Tamil-specific mappings."""
        pass  # Commented out as per instructions

    async def _load_telugu_mappings_from_db(self, session: AsyncSession):
        """Loads Telugu-specific mappings."""
        try:
            # Load Telugu consonants
            result = await session.execute(text("SELECT telugu_character, devanagari_character FROM consonants_telugu"))
            for row in result.fetchall():
                teluguChar, dev_char = row
                latin_equiv = self.devanagari_to_latin_consonant_map.get(
                    unicodedata.normalize('NFC', dev_char)
                )
                if latin_equiv:
            ...
            # Load Telugu vowels
            result = await session.execute(text("SELECT telugu_character, devanagari_character FROM vowels_telugu"))
            for row in result.fetchall():
                teluguChar, dev_char = row
                latin_equiv = self.devanagari_to_latin_vowel_map.get(
                    unicodedata.normalize('NFC', dev_char)
            ...
            # Load Telugu matras
            result = await session.execute(text("SELECT telugu_char, devanagari_char FROM telugu_matras_map"))
            for row in result.fetchall():
                telugu_matra, dev_matra = row
                dev_latin_equiv = self.DEVANAGARI_MATRA_TO_LATIN_VOWEL.get(
                    unicodedata.normalize('NFC', dev_matra)
                )
                if dev_latin_equiv:
            ...
            result = await session.execute(text("SELECT telugu_char, devanagari_char FROM telugu_modifiers_map"))
            for row in result.fetchall():
                telugu_mod, dev_mod = row
                dev_latin_equiv = self.DEVANAGARI_MODIFIERS_TO_LATIN.get(
                    unicodedata.normalize('NFC', dev_mod or "")
                )
                if dev_latin_equiv:
    ...
    async def add_mapping(self, session: AsyncSession, latin_char, insert_at, mapping_type, devanagari_char="", kannada_char=""):
        """Add a new mapping (consonant or vowel) to the database"""
        try:
